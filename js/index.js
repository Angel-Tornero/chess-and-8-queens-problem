...
function generateBoardMod() {
  const canvas = document.getElementById('board');
  new ChessBoard(12).draw(canvas);
  ...
  context.fillText('Este panel muestra la', 150, 150);
  context.fillText('posición de cada una de las', 150, 180);
  context.fillText('N reinas cuando se genera', 150, 210);
  context.fillText('una solución.', 150, 240);
  context.fillText('A mayor N, mayor', 150, 270);
  context.fillText('tiempo de espera', 150, 300);
}

function generateSolutionMod() {
  const canvas = document.getElementById('board');
  const board = new ChessBoard(12);
  const chessGame = new Chess(board);
  chessGame.drawBoard(canvas);
  ...
  for (let i = 0; i < solution.length; i++) {
    context.fillText('- ' + board.getPosition(solution[i]), 20, 40 + i * 30);
  }
}